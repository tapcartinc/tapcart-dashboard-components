...
const StyledBody = styled(Body)`
  ${props =>
    props.pagNav && `display: flex; flex-direction: row; align-items: center;`}
  ${media.sm`
    font-size: 16px;
    ${props =>
      props.pagNav &&
      `max-width: 20px; p {display:none} svg {width: 20px; margin: 0px;}`}
  `}
`;
...
const getPages = (
  ...
  if (currentPage === 1) {
    elements.push(
      <DisabledPage key={"prev"} {...props} pagNav>
        ...
        <StyledBody color={colors.grayText} pagNav>
          PREV
        </StyledBody>
      </DisabledPage>
    );
    ...
    elements.push(
      ...
      >
        ...
        <StyledBody color={colors.grayText} pagNav pagNav>
          PREV
        </StyledBody>
      </Page>
    );
  ...
  if (currentPage > 4) {
    elements.push(
      <>
        ...
        <StyledBody color={colors.grayText} pagNav color={colors.grayText}>
          ...
        </StyledBody>
      </>
    );
  ...
  if (currentPage < pagRange - 3) {
    elements.push(
      <>
        <StyledBody color={colors.grayText}>...</StyledBody>
  ...
  if (currentPage === pagRange) {
    elements.push(
      <DisabledPage {...props} key={"next"} pagNav>
        <StyledBody color={colors.grayText} pagNavpagNav>
          NEXT
        </StyledBody>
...
const Page = styled(StyledBody)`
  ...
  ${props => props.pagNav && `width: 60px; text-align: center`};
