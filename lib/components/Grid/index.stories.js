...
storiesOf('Examples|Components/Grid', module)
	...
	.add('Basic Grid', () => {
		...
		const items = Array.from(Array(numItems).keys()).map((i) => <div key={`item-${i}`}>{i + 1}</div>);
		return (
			<Grid
	...
	.add('Basic Column', () => {
		return (
			<Grid style={{ border: '1px solid #ddd' }} id="grid-example-column" type="column">
				<div>Grid item 1</div>
				<div>Grid item 2</div>
				<div>Grid item 3</div>
			</Grid>
	...
	.add('Wrapped Rows', () => {
		...
		// Create placeholder array of children
		const items = Array.from(Array(6).keys()).map((i) => <div key={`item-${i}`}>{i + 1}</div>);
		// render Grid
		return (
			<Grid id="grid-example-row-wrapped" type="row" wrap={true} wrapCount={3} margin={10} childStyle={itemStyle}>
				{items}
			</Grid>
		);
	});
