...
class AccordionSection extends Component {
  ...
  render() {
    ...
    // console.log("stepComplete", stepComplete(index));
    return (
          ...
          <div style={{ float: "right" }}>
            ...
            {isOpen && (
              <span>
                <Icon
                  type={
                    toggleIcons === "plus/minus" ? "small-close" : "arrow-up"
                  }
                ></Icon>
              </span>
            )}
...
const AccordionStyledItem = styled.div`
  ...
  border-radius: 3px;
  box-shadow: ${props =>
    props.boxShadow ? "0px 2px 4px 0px rgba(0,0,0,0.2)" : null};
  margin-bottom: 12px;
  ${props => (props.style ? props.style : null)};
`;

const AccordionLabel = styled.div.attrs({
  className: props => props.className || null
})`
  cursor: pointer;
  transition: 0.5s;
  display: flex;
...
const AccordionChildren = styled.div.attrs({
  className: props => `${props.className}__children` || null
})`
  margin-top: 10px;
  padding: 10px 0px;
  ${props =>
